...
@property (nonatomic) BOOL isTextFieldAlert;
@property (nonatomic) UITextField* gTextField;
@property (nonatomic) UILabel* gTitleLabel;
...
- (void)addScreenShot{
    ...
    if (JCiOS7OrLater) {
        ...
        originalImage = [UIImage imageWithCGImage:CGImageCreateWithImageInRect(viewImage.CGImage, CGRectMake(0, 20, 0.853333*[UIScreen mainScreen].bounds.size.width, 460))];
    }
...
+ (void)showTextFiledButtonsWithTitle:(NSString *)title Message:(NSString *)message Placeholder:(NSString*) placeholder ButtonTitle:(NSString *)buttonTitle Click:(clickHandleWithMessage)click ButtonTitle:(NSString *)buttonTitle1 Click:(clickHandleWithMessage)click1
{
    ...
    JCAlertView *alertView = [JCAlertView new];
    alertView.isTextFieldAlert = YES;
    alertView.placeholder = placeholder;
    [alertView configAlertViewPropertyWithTitle:title Message:message Buttons:@[@{[NSString stringWithFormat:@"%zi", JCAlertViewButtonTypeWarn] : buttonTitle}, @{[NSString stringWithFormat:@"%zi", JCAlertViewButtonTypeWarn] : buttonTitle1}] Clicks:@[newClick, newClick1] ClickWithIndex:nil];
}
...
+ (void)showTextFiledButtonsWithTitleAndText:(NSString *)title FieldText:(NSString*)text Message:(NSString *)message Placeholder:(NSString*) placeholder ButtonTitle:(NSString *)buttonTitle Click:(clickHandleWithMessage)click ButtonTitle:(NSString *)buttonTitle1 Click:(clickHandleWithMessage)click1
{
    ...
    alertView.placeholder = placeholder;
    [alertView configAlertViewPropertyWithTitle:title Message:message Buttons:@[@{[NSString stringWithFormat:@"%zi", JCAlertViewButtonTypeWarn] : buttonTitle}, @{[NSString stringWithFormat:@"%zi", JCAlertViewButtonTypeWarn] : buttonTitle1}] Clicks:@[newClick, newClick1] ClickWithIndex:nil];
    alertView.gTextField.autocorrectionType = UITextAutocorrectionTypeNo;
    alertView.gTextField.text = text;
}
...
- (void)alertBtnClick:(UIButton *)btn{
    [self dismissAlertWithCompletion:^{
        if (self.clicks.count > 0) {
            if (self.isTextFieldAlert)
            {
                clickHandleWithMessage handle = self.clicks[btn.tag];
                if (![handle isEqual:[NSNull null]]) {
                    handle(self.gTextField.text);
                }
            }else
            {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self name:UITextFieldTextDidChangeNotification object:self.gTextField];
    }];
}
...
- (void)setup{
    ...
    if(self.isTextFieldAlert)
    {
        self.center = CGPointMake(JCScreenWidth / 2, JCScreenHeight*0.35);
    }else
    {
        self.center = CGPointMake(JCScreenWidth / 2, JCScreenHeight / 2);
    }
    ...
    self.gTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(JCMargin, JCTITLEY, JCAlertViewWidth - JCMargin * 2, JCAlertViewTitleLabelHeight)];
    self.gTitleLabel.backgroundColor = [UIColor clearColor];
    self.gTitleLabel.text = self.title;
    self.gTitleLabel.textColor = MAIN_COLOR;
    self.gTitleLabel.font = JCAlertViewTitleFont;
    self.gTitleLabel.textAlignment = NSTextAlignmentCenter;
    [self addSubview:self.gTitleLabel];
    
    UILabel *contentLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, JCAlertViewTitleLabelHeight, JCAlertViewWidth - 20, JCAlertViewContentHeight)];
    ...
    if (self.isTextFieldAlert)//有输入框的AlertView
    {
        contentLabel.frame = CGRectMake(JCMargin, 12, JCAlertViewWidth - JCMargin * 2, JCAlertViewContentHeight);
        ...
        if (self.gTextField == nil)
        {
            ...
            self.gTextField.layer.borderColor = [UIColor lightGrayColor].CGColor;
            [self.gTextField setBackgroundColor:[UIColor clearColor]];
            [self.gTextField becomeFirstResponder];
             [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(textFieldEditChanged) name:UITextFieldTextDidChangeNotification object:self.gTextField];
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (i == 0)
            {
                UIImageView* imageView = [[UIImageView alloc] initWithFrame:CGRectMake(btn.frame.size.width+btn.frame.origin.x-0.5f, btn.frame.origin.y, .5, btn.frame.size.height)];
                [imageView setBackgroundColor:[UIColor lightGrayColor]];
                [self addSubview:imageView];
                
                UIImageView* TopLineView = [[UIImageView alloc] initWithFrame:CGRectMake(0, btn.frame.origin.y-.5, self.frame.size.width, .5)];
    ...
    }else
    {
        ...
        if (count == 1) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (i == 0)
                {
                    UIImageView* imageView = [[UIImageView alloc] initWithFrame:CGRectMake(btn.frame.size.width+btn.frame.origin.x-0.5f, btn.frame.origin.y, 0.5f, btn.frame.size.height)];
                    [imageView setBackgroundColor:[UIColor lightGrayColor]];
                    [self addSubview:imageView];
            ...
            for (int i = 0; i < count; i++) {
                float offset = i == (count-1)?0:0.5f;
                float soffset= i == 0?0:-0.5f;
                UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(soffset, JCAlertViewTitleLabelHeight + contentHeight + JCMargin + (JCMargin + JCButtonHeight) * i, JCAlertViewWidth - JCMargin * 2-offset, JCButtonHeight)];
                NSDictionary *btnDict = self.buttons[i];
                [btn setTitle:[btnDict.allValues firstObject] forState:UIControlStateNormal];
                [self setButton:btn BackgroundWithButonType:[[btnDict.allKeys firstObject] integerValue]];
                [self addSubview:btn];
                ...
                if (i < count-1)
                {
                    UIImageView* imageView = [[UIImageView alloc] initWithFrame:CGRectMake(btn.frame.size.width+btn.frame.origin.x-0.5f, btn.frame.origin.y, .5f, btn.frame.size.height)];
                    [imageView setBackgroundColor:[UIColor lightGrayColor]];
                    [self addSubview:imageView];
...
-(void)textFieldEditChanged{
    ...
    NSString *string=self.gTextField.text;
    if (string.length>maxLimit) {
        self.gTextField.text =[string substringToIndex:maxLimit];
    }
}
...
- (void)setButton:(UIButton *)btn BackgroundWithButonType:(JCAlertViewButtonType)buttonType{
    ...
    switch (buttonType) {
        ...
        case JCAlertViewButtonTypeCancel:
            ...
            //textColor = MAIN_COLOR;
            textColor = COLOR_RGB(27, 27, 27, 1);
            break;
        case JCAlertViewButtonTypeWarn:
            ...
            textColor = COLOR_RGB(27, 27, 27, 1);
            break;
        case JCAlertViewButtonTypeError:
            [self.gTitleLabel setTextColor:COLOR_RGB(254, 131, 125, 1)];
            normalImage = [UIImage imageNamed:@"JCAlertView.bundle/warn_nor"];
            highImage = [UIImage imageNamed:@"JCAlertView.bundle/warn_high"];
            textColor = COLOR_RGB(27, 27, 27, 1);
            break;
